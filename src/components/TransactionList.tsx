  ...
  return (
    <div className="bg-slate-900 rounded-2xl border border-slate-800/80 p-5 shadow-sm space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h3 className="font-bold text-white text-base">Riwayat Transaksi</h3>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2 text-xs bg-slate-800/80 border border-slate-700 rounded-xl text-white focus:bg-slate-850 focus:ring-2 focus:ring-emerald-500 focus:outline-none transition-all placeholder:text-slate-500"
        />
      </div>
      ...
      {filteredTransactions.length === 0 ? (
        <div className="text-center py-12 px-4 border border-dashed border-slate-800 rounded-xl bg-slate-900/50">
          ...
          <h4 className="text-sm font-semibold text-slate-300">Belum Ada Transaksi</h4>
        ...
        <div className="divide-y divide-slate-800/60 max-h-[520px] overflow-y-auto pr-1">
          {filteredTransactions.map((tx) => {
            const isIncome = tx.type === 'income';
            return (
              <div
                key={tx.id}
                className="py-3 px-2 flex items-center justify-between gap-3 hover:bg-slate-800/50 rounded-xl transition-colors group"
              >
                {/* Left: Icon & Info */}
      ...
      >
        {txToDelete && (
          <div className="bg-slate-800/80 border border-slate-700 rounded-xl p-3.5 flex items-center justify-between gap-3 text-xs">
            <div className="min-w-0">
              <span className="font-bold text-white block text-sm">
                {txToDelete.category || (txToDelete.type === 'income' ? 'Pemasukan' : 'Pengeluaran')}
              </span>
              {txToDelete.note && (
                <span className="text-slate-400 block text-xs truncate mt-0.5">
                  {txToDelete.note}
                </span>
              )}
            ...
            <div className="text-right shrink-0">
              <span
                className={`font-bold text-sm block ${
                  txToDelete.type === 'income' ? 'text-emerald-400' : 'text-rose-400'
                }`}
              >
